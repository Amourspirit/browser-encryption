...
const loadCSSLinksAsync = async (stylesheets) => {
  let arr = await Promise.all(stylesheets.map(linkinfo => fetch(linkinfo.link)))
  arr = await Promise.all(arr.map(linkinfo => linkinfo.text()))
 ...
 */
methods.loadCSSLinksJson = (jsonUrl) => {
  fetch(jsonUrl)
    .then(res => res.json())
    .then((out) => {
      //console.log('Output: ', out);
      methods.loadCSSLinks(out.imports)
  ...
  const processContent = () => {
    ...
    if (isValidContent) {
      ...
      if (window.USE_LAZY_LOAD) {
        ...
        $(contentEl).append($(html).bsResponsive().newWindow());
      }
